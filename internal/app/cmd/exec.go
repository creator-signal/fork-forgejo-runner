...
func loadExecCmd(ctx context.Context) *cobra.Command {
	...
	execCmd.PersistentFlags().StringVarP(&execArg.containerDaemonSocket, "container-daemon-socket", "", "", "Path to Docker daemon socket which will be mounted to containers (e.g.: /var/run/docker.sock). If not specified or invalid, will scan the system for common socket paths.")
