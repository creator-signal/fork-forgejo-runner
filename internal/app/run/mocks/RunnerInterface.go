...
// MemoryScheduler provides a mock function with no fields
func (_m *RunnerInterface) MemoryScheduler() firecracker.Scheduler {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for MemoryScheduler")
	}

	var r0 firecracker.Scheduler
	if rf, ok := ret.Get(0).(func() firecracker.Scheduler); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(firecracker.Scheduler)
		}
	}
