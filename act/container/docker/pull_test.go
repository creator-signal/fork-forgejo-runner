...
func TestGetImagePullOptions(t *testing.T) {
	...
	t.Run("Error without platform", func(t *testing.T) {
		_, err := getImagePullOptions(ctx, NewDockerPullExecutorInput{})
		require.ErrorContains(t, err, `malformed container platform: ""`)
	})

	t.Run("Empty registry credentials", func(t *testing.T) {
		options, err := getImagePullOptions(ctx, NewDockerPullExecutorInput{Platform: "linux/amd64"})
		require.NoError(t, err)

		assert.Empty(t, options.RegistryAuth)
	})

	t.Run("Registry credentials from arguments", func(t *testing.T) {
		...
		assert.Equal(t, "eyJ1c2VybmFtZSI6InVzZXJuYW1lIiwicGFzc3dvcmQiOiJwYXNzd29yZCJ9", options.RegistryAuth)
	})

	t.Run("Registry credentials from Docker configuration", func(t *testing.T) {
		...
		assert.Equal(t, "eyJ1c2VybmFtZSI6InVzZXJuYW1lIiwicGFzc3dvcmQiOiJwYXNzd29yZFxuIiwic2VydmVyYWRkcmVzcyI6Imh0dHBzOi8vaW5kZXguZG9ja2VyLmlvL3YxLyJ9", options.RegistryAuth)
	})
}
