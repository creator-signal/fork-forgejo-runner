...
type Client struct {
	...
	rpc  pluginv1alpha.BackendPluginClient
...
// SERVING before the caller issues RPCs over it.
...
func NewClient(ctx context.Context, address string, options ...ClientOption) (*Client, error) {
	...
	caps, err := rpc.Capabilities(ctx, &pluginv1alpha.CapabilitiesRequest{})
	if err != nil {
		conn.Close()
...
func validateCapabilities(caps *pluginv1alpha.CapabilitiesResponse) error {
	missing := []string{}
	if caps.GetName() == "" {
