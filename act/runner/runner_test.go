...
func TestPullFailureIsJobFailure(t *testing.T) {
	...
	for _, table := range tables {
		t.Run(table.workflowPath, func(t *testing.T) {
			...
			table.runTest(common.WithLogger(WithJobLoggerFactory(context.Background(), factory), logger), t, config)
...
func (c mockCache) Fetch(ctx context.Context, cacheDir, url, ref, token string) (string, error) {
	_ = ctx
	_ = cacheDir
	_ = url
	_ = ref
...
func (c mockCache) GetTarArchive(ctx context.Context, cacheDir, sha, includePrefix string) (io.ReadCloser, error) {
	_ = ctx
	_ = cacheDir
	_ = sha
	_ = includePrefix
